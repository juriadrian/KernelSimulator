...
class CpuModel:
    ...
    def execute(self):
        ...
        while count > 0 or self.quantum == -1:
            instruction = self.kernel.logical_memory.get_instruction_of_cell(self.pcb.pc, self.pcb.block_id)#En vez de usar memory position usaremos el PROGRAM COUNTER
            if instruction == 'EOF':
                #setState con enum
